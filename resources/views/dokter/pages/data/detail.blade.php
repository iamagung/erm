                ...
                <table class="table table-bordered table-striped" id="dataPoli">
                    ...
                    <tbody id="hasilCari">
                        ...
                        <tr>
                            ...
                            <td>{!! $key->NamaPoli !!}</td>
                            <td>
                                <!-- if($key->KodePoli==$dokter->poli_id) -->
                                <?php
...
<script type="text/javascript">
    function page(i){
        ...
        $.post("{!! route('pageDataDetailDokter') !!}",{no_RM:no_RM,cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('pageDataDetailDokter') !!}",{no_RM:no_RM,cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('pageDataDetailDokter') !!}",{no_RM:no_RM,cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
    ...
    function tampilkan(data){
        ...
        if(d==0){
            ...
            '<td>';
                if(value.KodePoli=='{{$dokter->poli_id}}'){
                    <?php
                    if(Session::has('id_rekap')){
                        ?>
                        if(value.id_rekapMedik=='<?php echo Session::get('id_rekap');?>'){
                            ...
                            html+='<a href="{!! url("/") !!}/rekap_medik/ganti/'+value.id_rekapMedik+'" class="btn btn-sm btn-primary">Ganti</a>';
                        }
                        <?php
                    }else{
                        ?>
