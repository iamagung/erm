    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="col-lg-12 col-md-12">
                <a href="#" class="btn btn-primary" style="border-radius: 10px !important" onclick="formDokter(0)"><i class="fa fa-plus"> Tambah Dokter Poli</i></a>
            </div>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="javascript:void(0);" onclick="viewDokter('{!! $key->i !!}')"><i class="fa fa-eye"></i> View</a></li>
                                        <li><a href="#" onclick="formDokter('{!! $key->i !!}')"><i class="fa fa-edit"></i> Edit</a></li>
                                        <li><a href="#" onclick="resetDokter('{!! $key->i !!}')"><i class="fa fa-refresh"></i> Reset Password</a></li>
                                        <li><a href="javascript:void(0);" onclick="deleteDokter('{!! $key->i !!}')"><i class="fa fa-trash-o"></i> Delete</a></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
...
<script type="text/javascript">
    ...
    function page(i){
        ...
        $.post("{!! route('pageDokter') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('pageDokter') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('pageDokter') !!}",{cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
            ...
            '<td>'+
                '<div class="btn-group">'+
                    ...
                    '<ul class="dropdown-menu pull-right">'+
                        ...
                        '<li><a href="#" onclick="formDokter('+value.i+')"><i class="fa fa-edit"></i> Edit</a></li>'+
    ...
    function formDokter(id){
        $.post("{!! route('formDokter') !!}",{id:id}).done(function(data){
            if(data.status=='success'){
    ...
    function viewDokter(id){
        $.post("{!! route('detailDokter') !!}",{id:id}).done(function(data){
            if(data.status=='success'){
                $('.modal_layout').html(data.content);
                $('.modal_layout').show();
                $('#myModal').modal('show');
    ...
    function deleteDokter(id){
        ...
        function(){
            $.post("{!! route('deleteDokter') !!}",{id:id}).done(function(data){
                if(data.status=='success'){
    ...
    function resetDokter(id){
        ...
        function(){
            $.post("{!! route('resetDokter') !!}",{id:id}).done(function(data){
                if(data.status=='success'){
                    swal("Berhasil!", "Data berhasil dihapus!", "success");
                }else{
                    swal("Gagal!", "Data gagal dihapus!", "error");
