                ...
                <div class="col-lg-8 col-md-8" style="padding: 0px">
                    ...
                    <div class="col-lg-6 col-md-6">
                        <select class="form-control" name="searchby" style="border-radius: 10px !important">
                            ...
                            <option value="DariPoli">Nama Poli</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-bordered table-striped" id="dataPoli">
                    ...
                    <tbody id="hasilCari">
                        ...
                        <tr>
                            ...
                            <td>{!! $key->DariPoli !!}</td>
                            <td>
                                <a href="{{url('dokter/rujuk/'.$key->id_rujukan)}}" class="btn btn-sm btn-primary">Jawab</a>
...
<script type="text/javascript">
    function page(i){
        ...
        $.post("{!! route('page_jawab_pertanyaan') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('page_jawab_pertanyaan') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('page_jawab_pertanyaan') !!}",{cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
    ...
    function tampilkan(data){
        ...
        if(d==0){
            ...
            '<td>'+value.DariPoli+'</td>'+
            '<td>'+
                '<a href="{!! url("/") !!}/dokter/rujuk/'+value.id_rujukan+'" class="btn btn-sm btn-primary">Jawab</a>'+
