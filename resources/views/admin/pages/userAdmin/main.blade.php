                ...
                <div class="col-lg-8 col-md-8" style="padding: 0px">
                    ...
                    <div class="col-lg-6 col-md-6">
                        <select class="form-control" name="searchby" style="border-radius: 10px !important">
                            <option value="nama">Nama Admin</option>
                    ...
                    <tbody id="hasilCari">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if($key->aktif=='Y'){
                                    ?>
                                    <i class="btn btn-sm btn-primary"> Aktif</i>
                                    <?php
                                }else{
                                    ?>
                                    <i class="btn btn-sm btn-danger"> Tidak Aktif</i>
                                    <?php
                                }
                                ?>
                            </td>
                            <td>
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="javascript:void(0);" onclick="viewAdmin('{!! $key->id !!}')"><i class="fa fa-eye"></i> View</a></li>   
                                        <li><a href="#" onclick="formAdmin('{!! $key->id !!}')"><i class="fa fa-edit"></i> Edit</a></li>
                                        <li><a href="#" onclick="resetAdmin('{!! $key->id !!}')"><i class="fa fa-refresh"></i> Reset Password</a></li>
                                        <li><a href="javascript:void(0);" onclick="deleteAdmin('{!! $key->id !!}')"><i class="fa fa-trash-o"></i> Delete</a></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
...
<script type="text/javascript">
    ...
    function page(i){
        ...
        $.post("{!! route('pageAdmin') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('pageAdmin') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('pageAdmin') !!}",{cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
    ...
    function tampilkan(data){
        ...
        if(d==0){
            ...
            '<td>'+value.username+'</td>'+
                '<td>';
                    if(value.aktif=='Y'){
                        html+='<i class="btn btn-sm btn-primary"> Aktif</i>';
                    }else{
                        html+='<i class="btn btn-sm btn-danger"> Tidak Aktif</i>';
                    }
                html+= '</td>'+
                '<td>'+
                    '<div class="btn-group">'+
                        '<a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-bars"></i></a>'+
                        '<ul class="dropdown-menu pull-right">'+
                            '<li><a href="javascript:void(0);" onclick="viewAdmin('+value.id+')"><i class="fa fa-eye"></i> View</a></li>'+   
                            '<li><a href="#" onclick="formAdmin('+value.id+')"><i class="fa fa-edit"></i> Edit</a></li>'+
                            '<li><a href="#" onclick="resetAdmin('+value.id+')"><i class="fa fa-refresh"></i> Reset Password</a></li>'+
                            '<li><a href="javascript:void(0);" onclick="deleteAdmin('+value.id+')"><i class="fa fa-trash-o"></i> Delete</a></li>'+
                        '</ul>'+
                    '</div>'+
                '</td>'+
            '</tr>';
    ...
    function formAdmin(id){
        $.post("{!! route('formAdmin') !!}",{id:id}).done(function(data){
            if(data.status=='success'){
    ...
    function viewAdmin(id){
        $.post("{!! route('detailAdmin') !!}",{id:id}).done(function(data){
            if(data.status=='success'){
                $('.modal_layout').html(data.content);
                $('.modal_layout').show();
                $('#myModal').modal('show');
    ...
    function deleteAdmin(id){
        ...
        function(){
            $.post("{!! route('deleteAdmin') !!}",{id:id}).done(function(data){
                if(data.status=='success'){
    ...
    function resetAdmin(id){
        swal({
        title: "Anda yakin?",
        text: "Password akan direset seperti username!",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: '#DD6B55',
        confirmButtonText: 'Ya, hapus!',
        ...
        function(){
            $.post("{!! route('resetAdmin') !!}",{id:id}).done(function(data){
                if(data.status=='success'){
                    swal("Berhasil!", "Data berhasil dihapus!", "success");
                }else{
                    swal("Gagal!", "Data gagal dihapus!", "error");
