    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="col-lg-12 col-md-12">
                <a href="javascript:void(0);" class="btn btn-primary" style="border-radius: 10px !important" onclick="formDokter(0)"><i class="fa fa-plus"> Tambah Perawat Poli</i></a>
            </div>
                ...
                <table class="table table-bordered table-striped" id="dataPoli">
                    ...
                    <tbody id="hasilCari">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    <a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-bars"></i></a>
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="javascript:void(0);" onclick="viewDokter('{!! $key->id !!}')"><i class="fa fa-eye"></i> View</a></li>   
                                        <li><a href="javascript:void(0);" onclick="formDokter('{!! $key->id !!}')"><i class="fa fa-edit"></i> Edit</a></li>
                                        <li><a href="javascript:void(0);" onclick="resetDokter('{!! $key->id !!}')"><i class="fa fa-refresh"></i> Reset Password</a></li>
...
<script type="text/javascript">
    ...
    function page(i){
        ...
        $.post("{!! route('pagePerawat') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('pagePerawat') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('pagePerawat') !!}",{cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
        ...
        if(d==0){
            ...
            '<td>'+
                '<div class="btn-group">'+
                    '<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-bars"></i></a>'+
                    '<ul class="dropdown-menu pull-right">'+
                        '<li><a href="javascript:void(0);" onclick="viewDokter('+value.id+')"><i class="fa fa-eye"></i> View</a></li>'+   
                        '<li><a href="javascript:void(0);" onclick="formDokter('+value.id+')"><i class="fa fa-edit"></i> Edit</a></li>'+
                        '<li><a href="javascript:void(0);" onclick="resetDokter('+value.id+')"><i class="fa fa-refresh"></i> Reset Password</a></li>'+
                        '<li><a href="javascript:void(0);" onclick="deleteDokter('+value.id+')"><i class="fa fa-trash-o"></i> Delete</a></li>'+
                    '</ul>'+
                '</div>'+
            '</td>'+
            '</tr>';
    ...
    function formDokter(id){
        $.post("{!! route('formPerawat') !!}",{id:id}).done(function(data){
            if(data.status=='success'){
    ...
    function viewDokter(id){
        $.post("{!! route('detailPerawat') !!}",{id:id}).done(function(data){
            if(data.status=='success'){
                $('.modal_layout').html(data.content);
                $('.modal_layout').show();
                $('#myModal').modal('show');
    ...
    function deleteDokter(id){
        ...
        function(){
            $.post("{!! route('deletePerawat') !!}",{id:id}).done(function(data){
                if(data.status=='success'){
    ...
    function resetDokter(id){
        ...
        function(){
            $.post("{!! route('resetPerawat') !!}",{id:id}).done(function(data){
                if(data.status=='success'){
                    swal("Berhasil!", "Data berhasil dihapus!", "success");
                }else{
                    swal("Gagal!", "Data gagal dihapus!", "error");
