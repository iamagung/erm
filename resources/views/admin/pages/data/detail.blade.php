...
<script type="text/javascript">
    function page(i){
        ...
        $.post("{!! route('pageDataDetail') !!}",{no_RM:no_RM,cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('pageDataDetail') !!}",{no_RM:no_RM,cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('pageDataDetail') !!}",{no_RM:no_RM,cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
            ...
            '<td>';
                ...
                if(Session::has('id_rekap')){
                    ...
                    if(value.id_rekapMedik=='<?php echo Session::get('id_rekap');?>'){
                        ...
                        html+='<a href="{!! url("/") !!}/rekap_medik/gantiAdmin/'+value.id_rekapMedik+'" class="btn btn-sm btn-primary">Ganti</a>';
                    }
                    <?php
                }else{
                    ?>
