...
<body>
			...
			<td width="45%">
				<table width="100%">
					...
					<tr>
						...
						<td>{{ date('d-m-Y',strtotime($syarat->tanggalKunjungan)) }}</td>
					</tr>
				</table>
			</td>
		</tr>
			...
			<td style="border: 1px solid #000">
				<table width="100%" border="1">
					...
					<tr>
						...
						<td>
							<h3>Perawat</h3>
							<p style="margin-left: 15px">{{$syarat->nama_perawat}}</p>
							<h3>Dokter</h3>
							<p style="margin-left: 15px">{{$syarat->NamaDokter}}</p>
						</td>
					</tr>
				</table>
			</td>
				...
				<table width="100%">
					<tr>
						<td style="border:1px solid #000;width: 50%">
							Anamnesis (S) dan Pemeriksaan Fisik (O)<br/>
							AnamNesis (S)<br>
							<?php
							...
							?>
							<br/>
							<p>{{$syarat->pemeriksaanFisik}}</p><br/>
							...
							<div style="width: 100%;border: 1px solid #000">
								...
								if($syarat->tindakan!=''){
									...
									<ol>
										<?php
										$d = explode(";", $syarat->tindakan);
										$k = explode(";", $syarat->icd9);
										for ($i=0; $i < count($d)-1; $i++) {
											?>
											<li>{{$d[$i]}} ({{$k[$i]}})</li>
											<?php
				            ...
				            <div>
				            	<?php
								$resep = DB::connection('rsu')->table('tr_rawatjalanobat')->where('No_Register',$syarat->no_Register)->get();
								?>
								Obat E-Resep
	...
	<div style="float:right">
		Mojokerto, {{date('d-m-Y',strtotime($syarat->tanggalKunjungan))}}
		<br>
		Dokter Pemeriksa
		<br>
		<?php
		if($syarat->KodeDokter!=''){
			$dkt = DB::table('login_dokter')->where('dokter_id',$syarat->KodeDokter)->first();
			if(!empty($dkt)){
				if($dkt->ttd!=''){
					if(file_exists('ttd/'.$dkt->ttd)){
						?>
						<img src="{{ asset('ttd/'.$dkt->ttd) }}" alt="Tanda tangan" style="width:3cm">
						<?php
					}
				}
				echo "<br>";
