	...
	<div class="col-lg-12 col-md-12 tahap7" style="padding: 0px">
		...
		<table class="table table-bordered">
			...
			<tbody>
                <tr>
                    <th>Keterangan</th>
                    <th width="50%"><a href="http://192.168.1.234/condc/stable/viewers/static/index.php?id=http://192.168.1.234/condc/stable/ad.dcm" target="_blank">ad.dcm</a></th>
...
<script type="text/javascript">
    function tambahRadio(){
        $.post("{!! route('modalTambahRadio') !!}",{}).done(function(data){
            if(data.status=='success'){
                $('.modal_layout_radio').html(data.content);
                $('.modal_layout_radio').show();
                $('#myModalRadio').modal('show');
            }
        });
    }
    ...
    function deleteRadio(i){
        ...
        function(){
            $.post("{!! route('hapusRadio') !!}",{id:i}).done(function(data){
                if(data.status=='success'){
