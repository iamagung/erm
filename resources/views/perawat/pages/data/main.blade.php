                ...
                <table class="table table-bordered table-striped" id="dataPoli">
                    ...
                    <tbody id="hasilCari">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{!! url('perawat/data_rekap_medik/detailRekapPasien/'.$key->no_RM) !!}" class="btn btn-sm btn-primary">Detail</a>
                            </td>
                        </tr>
                        <?php
                        $i++;
...
<script type="text/javascript">
    ...
    function page(i){
        ...
        $.post("{!! route('pageDataDokterPerawat') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:i}).done(function(data){
            $('#hasilCari').html('');
    ...
    $('select[name=searchby]').change(function(){
        ...
        $.post("{!! route('pageDataDokterPerawat') !!}",{cariText:cariText,cariStatus:cariStatus,by:searchby,i:1}).done(function(data){
    ...
    $('#searchtext').keyup(function(){
        ...
        $.post("{!! route('pageDataDokterPerawat') !!}",{cariText:cariText,cariStatus:cariStatus,by:by,i:1}).done(function(data){
            if(data.status=='success'){
                $('#hasilCari').html('');
                $('#hasilCari').html(tampilkan(data.data));
                $('.pagination').html(halaman_page(1,data.data));
    ...
    function tampilkan(data){
        ...
        if(d==0){
            ...
            '<td>'+
                '<a href="{!! url("/") !!}/perawat/data_rekap_medik/detailRekapPasien/'+value.no_RM+'" class="btn btn-sm btn-primary">Detail</a>'+
            '</td>'+
            '</tr>';
            i++;
            });
