	...
	<div class="box">
		...
		<div class="box-body">
					  ...
					  <input type="hidden" name="fr-no_R" id="fr-no_R" value="">
						...
						<tr id="fr-1">
							...
							<td>
							  ...
							  <input class="data-list-txt form-control" onkeydown="ModifyEnterKeyPressAsTab(event);" type="text" value="" name="petunjuk_khusus_1" id="petunjuk_khusus_1">
			...
			<div class="col-lg-4 col-md-4">
				  <button type="button" name="button" class="btn btn-success" id="list_pr"><i class="fa fa-list" aria-hidden="true"></i><span style="margin-left:5px;">List Paket Resep</span></button>
				  <button type="button" name="button" class="btn btn-primary" id="simpan_pr"><i class="fa fa-save" aria-hidden="true"></i><span style="margin-left:5px;">Simpan Paket Resep</span></button>
				</div>
			<div class="col-lg-8 col-md-8">
				  <button type="button" name="button" class="btn btn-success" id="add_fr">Tambah baris</button>
				  <button type="button" class="btn btn-danger btn-md" onclick="delResepItem();">
					  <i class="fa fa-trash-o" aria-hidden="true"></i> Hapus
				  </button>
				  <button type="button" name="button" class="btn btn-primary" id="save_fr">Simpan obat</button>
			</div>
...
<script type="text/javascript">
	$("#add_fr").click(function(event) {
		...
		var el = `
			...
			<input class="data-list-txt form-control" onkeydown="ModifyEnterKeyPressAsTab(event);" type="text" value="" name="petunjuk_khusus_`+fr_count+`" id="petunjuk_khusus_`+fr_count+`">
	...
	function setObat2(kode,nama,KodeGd,satuan,no,saldo){
		...
		$('#kode_obat_'+no).val(kode);
		$('#nama_obat_'+no).val(nama);
		$('#kode_gd_'+no).val(KodeGd);
		$('#satuan_'+no).val(satuan);
		$('#saldo_'+no).val(saldo);
	...
	$(document.body).on('keyup', '.kode_obat' ,function(){
	  ...
	  if (cari == '') {
	  	...
	  	$.post("{{route('get_kode_resep')}}",data,function(data){
	  		...
	  		if(data.data.length!=0){
	  			$.each(data.data,function(index,value){
	  				tag += '<a href="javascript:void(0)" onclick="setObat2(\''+value.item_code+'\',\''+value.value+'\',\''+value.unit_nickname+'\',\''+value.item_unitofitem+'\','+no+',\''+value.stock+'\')">'+value.value+' <span style="color:#000;">(Stok : '+value.stock+')</span></a><br>';
